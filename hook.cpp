...
NTSTATUS Hook::Handler(PVOID callParam)
{
    ...
    case CMD_READ_BATCH:
    {
        /* buffer points to a usermode BATCH_READ_REQUEST; size = output buffer size in bytes */
        if (!req->buffer || !MmIsAddressValid((PVOID)req->buffer)) {
            req->result = 0;
            break;
        }
        PBATCH_READ_REQUEST batchReq = (PBATCH_READ_REQUEST)req->buffer;
        if (!MmIsAddressValid(batchReq) || batchReq->count == 0 ||
            batchReq->count > BATCH_MAX_ENTRIES || !batchReq->outBuffer) {
            req->result = 0;
            break;
        }
        req->result = PhysicalBatchReadProcessMemory(
            (HANDLE)req->pid,
            batchReq,
            (SIZE_T)req->size   /* caller sets size = total output buffer bytes */
        ) ? 1 : 0;
        break;
    }
